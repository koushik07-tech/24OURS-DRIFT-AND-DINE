...
export default function SmokeCanvas({ stage, intensity = 1.0 }: SmokeCanvasProps) {
  ...
  const { particleMultiplier, isMobile } = useDeviceCapability();
  ...
  useEffect(() => {
    ...
    const maxParticles = Math.floor(200 * particleMultiplier);
    ...
    const createParticle = (typeOverride?: "smoke" | "streak" | "ember"): Particle => {
      ...
      if (isStreak) {
        return {
          ...
          type: "streak",
          length: Math.random() * (isMobile ? 90 : 180) + 90,
        };
      }
    ...
    if (stage >= 1) {
      const initCount = Math.floor(30 * particleMultiplier);
    ...
    const render = () => {
      ...
      let baseSpawn = 0;
      if (stage === 1) baseSpawn = 1;
      else if (stage === 2) baseSpawn = 5;
      else if (stage === 3) baseSpawn = 7;
      else if (stage === 4) baseSpawn = 2;
      else if (stage === 5) baseSpawn = 1;

      const spawnCount = Math.round(baseSpawn * intensity * particleMultiplier);
  ...
  }, [stage, intensity, particleMultiplier, isMobile]);
