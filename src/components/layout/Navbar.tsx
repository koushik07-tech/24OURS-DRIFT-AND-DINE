...
import { Menu, X, Calendar, User, ShieldAlert, LogOut, LayoutDashboard, Flag } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 flex flex-col">
      {/* Top Announcement Banner (Student Offer & Launch Special) */}
      <AnnouncementBanner />
      ...
      <div
        className={`w-full transition-all duration-300 ${
          isScrolled || pathname !== "/"
            ...
            : "bg-gradient-to-b from-brand-black/95 via-brand-black/50 to-transparent py-4 sm:py-5"
        }`}
      >
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group shrink-0">
              ...
              <div className="text-left hidden xs:block sm:block">
                ...
                <span className="text-[8px] sm:text-[9px] font-mono tracking-[0.2em] text-brand-red font-bold uppercase block">
                  DRIFT AND DINE
                </span>
              </div>
            </Link>
            ...
            {/* Desktop Nav Links */}
            <nav className="hidden 2xl:flex items-center gap-1 px-3 py-1.5 rounded-full bg-carbon-900/80 border border-white/10 backdrop-blur-md">
              {siteConfig.navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="px-3 py-1 text-[11px] font-mono uppercase tracking-wider rounded-full text-carbon-300 hover:text-white hover:bg-white/10 transition-colors"
                >
                  {link.name}
                </a>
              ))}
            ...
            {/* Prominent 28-Minute Countdown Timer in Header */}
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-48 rounded-2xl bg-carbon-900 border border-white/15 shadow-2xl py-2 z-50 animate-fadeIn">
                      <div className="px-4 py-2 border-b border-white/10 text-[11px] font-mono text-carbon-400">
                        Driver Profile: <span className="text-white block truncate">{user?.email}</span>
                      </div>
              ...
              <button
                ...
                className="xl:hidden p-2.5 rounded-lg bg-carbon-850 border border-white/10 text-carbon-200 hover:text-white"
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="xl:hidden bg-brand-black/98 border-b border-white/15 backdrop-blur-2xl px-4 pt-4 pb-8 space-y-4 max-h-[80vh] overflow-y-auto">
          <nav className="flex flex-col space-y-1">
            {siteConfig.navLinks.map((link) => (
              <a
                ...
                className="px-4 py-2.5 rounded-lg text-xs font-mono font-semibold uppercase tracking-wider text-carbon-200 hover:text-white hover:bg-carbon-850"
              >
                {link.name}
              </a>
            ))}
